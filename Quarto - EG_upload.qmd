...
hg <- ggplot(data, aes(x=value)) +
  geom_histogram(bins = 100, fill = 'gray66', color = 'gray26') +
...
We're now creating a new column within our data - logarithmic values of our previous values.

```{r}
data$log_value <- log(data$value)
```

We will now create a violin plot called vp, what has values on the x-axis and the newly created logarithmic values on the y axis.
...
In all honesty R is not my strength and I am still far from fully understanding how certain functions work. The weakest point in this exercise is the violin plot as it does not make sense to me completely.
